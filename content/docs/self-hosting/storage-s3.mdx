...
By default, Riffado stores audio on the local filesystem (Docker
...
Riffado talks to any service that implements the S3 wire protocol —
...
If MinIO runs in the same Docker network as Riffado, the endpoint is
...
Riffado's at-rest encryption applies to **database** fields, not audio
...
Riffado does not ship a built-in "move all audio from local to S3"
