...
Riffado ships as a single Docker image on GitHub Container Registry.
...
`RIFFADO_VERSION` in your `.env` controls which image tag Compose
pulls.
...
above. See [BRANCHING.md](https://github.com/riffado/riffado/blob/main/BRANCHING.md)
...
To roll back, set `RIFFADO_VERSION` to the previous tag and re-run
the upgrade command:

```bash
# In .env
RIFFADO_VERSION=0.4.1
```
...
This works because Riffado's database migrations are
